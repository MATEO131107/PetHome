...
function exportarVacunasPDF() {
  const { jsPDF } = window.jspdf;
  const doc = new jsPDF();

  doc.setFontSize(16);
  doc.text("Vacunas de la mascota", 20, 20);

  doc.setFontSize(12);
  doc.text("Rabia", 20, 40);
  doc.text("Parvovirus", 20, 50);
  doc.text("Moquillo", 20, 60);

  doc.text("Mantener las vacunas al dia protege la salud de tu mascota.", 20, 80);

  doc.save("vacunas.pdf");
}

function exportarMascotaPDF() {
  const { jsPDF } = window.jspdf;
  const doc = new jsPDF();

  doc.setFontSize(16);
  doc.text("Ficha de Max", 20, 20);

  doc.setFontSize(12);
  doc.text("Nombre: Max", 20, 40);
  doc.text("Edad: 3 años", 20, 50);
  doc.text("Raza: Labrador", 20, 60);
  doc.text("Peso: 25 kg", 20, 70);
